...
// GoogleChatNotifier sends port change events to a Google Chat webhook.
type GoogleChatNotifier struct {
	webhookURL string
	client     *http.Client
}

// NewGoogleChatNotifier creates a notifier that posts to a Google Chat incoming webhook.
...
func (n *GoogleChatNotifier) Notify(events []alert.Event) error {
	...
	buf.WriteString(fmt.Sprintf("*portwatch alert* — %d port change(s) detected\n", len(events)))
